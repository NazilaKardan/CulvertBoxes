% Scour Depth Prediction using ANFIS optimized with ECBO
...
fis = genfis1(train_data, 3, 'gbellmf'); % 3 Gaussian bell membership Sony
fis = anfis(train_data, fis, [100 0 0.01 0.9 1.1], 0); % Initial training

%% Step 3: Enhanced Colliding Bodies Optimization (ECBO)
% ECBO Parameters
pop_size = 20; % Number of colliding bodies
max_iter = 1000; % Maximum iterations
Pro = 0.35; % Probability of mutation
CMS = 10; % Colliding memory size
...
memory = zeros(CMS, n_params); % Memory for best solutions
memory_fitness = inf(CMS, 1);
best_fitness = inf;
...
for iter = 1:max_iter
    ...
    for i = 1:pop_size
        ...
        % Update memory
        if fitness(i) < max(memory_fitness)
            idx = find(memory_fitness == max(memory_fitness));
            memory(idx(1),:) = pop(i,:);
            memory_fitness(idx(1)) = fitness(i);
        end
    end
    ...
    for i = 1:pop_size
        if rand < Pro
            j = randi(n_params); % Random dimension
            pop(i,j) = lb(j) + (ub(j) - lb(j)) * rand; % Random mutation
        end
    end
end
...
% Display Results
fprintf('ANFIS-ECBO Training Metrics:\n');
fprintf('R²: %.4f\n', r2_train);
fprintf('RMSE: %.4f\n', rmse_train);
fprintf('MAE: %.4f\n', mae_train);
fprintf('MAPE: %.4f%%\n', mape_train);
fprintf('ANFIS-ECBO Testing Metrics:\n');
fprintf('R²: %.4f\n', r2_test);
fprintf('RMSE: %.4f\n', rmse_test);
fprintf('MAE: %.4f\n', mae_test);
fprintf('MAPE: %.4f%%\n', mape_test);
...
xlabel('Sample'); ylabel('Scour Depth (dse/dₒ)'); title('ANFIS-ECBO: Observed vs Predicted');
legend('show'); grid on;

subplot(2,1,2);
plot(1:length(test_output), test_output - test_pred, 'k-', 'LineWidth', 1.5);
xlabel('Sample'); ylabel('Error (Observed - Predicted)'); title('ANFIS-ECBO: Prediction Error');
grid on;

%% Step 7: Save the Model
save('ANFIS_ECBO_Model.mat', 'optimized_fis');
fprintf('Model saved as ANFIS_ECBO_Model.mat\n');
